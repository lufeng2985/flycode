...
YouYou may have noticed that `ref` has two methods to force a provider to recompute, and wonder how they differ.
...
The solution to this problem is typically to move your logic into a `Notifier` (see [Providers](/docs/concepts2/providers)), and then have your logic be a method of that `Notifier`.
...
*   It is not possible to implement an "on change" listener with `InheritedWidget`.
    ...
    One issue is that the life-cycle can be triggered even if no dependency changed, especially if your widget tree uses GlobalKeys (and some Flutter widgets already do so internally).
...
*   Widgets listening to an `InheritedWidget` never stop listening to it.
    ...
    When the page offset changes, you wouldn't want your widget to keep listening to the previously visible pages.
...
Maybe in a distant future, those issues will be fixed. In that case, Riverpod would migrate to using `BuildContext` instead of `Ref`.
